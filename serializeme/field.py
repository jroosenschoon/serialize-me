...
"""
from serializeme.exceptions import InvalidSize, ValueTooBig, InvalidValue, InvalidIPv4Address
from serializeme import IPV4, HOST, NULL_TERMINATE, PREFIX_LENGTH, PREFIX_LEN_NULL_TERM, SPECIAL_FIELD_TYPES
...
class Field:
    ...
    def __init__(self, name, size=1, value=0):
        """
        Field object to store a value with a specified size for some packet.

        Parameters:
            name : (str)
                   The name of the field.

            size : (int, str), default=1
                   The number of bits of the field, or a string denoting a
                   custom sized field.

            value : (int, bytes, str), default=0
                  The value of the field.
        """
        # Size can be number of bits, string of bits/bytes, or string of CONST.
        if type(size) is int:
            if type(value) is int:
                ...
                if self.__can_fit(size, value):
                    if size % 8 == 0:
                        ...
                        # so we will do just that. The size is the number of
                        # times 8 goes into the number of bits.
                        self.size = size // 8
            ...
            # Currently when size is str, it must be one of the specified
            # values.
            if size == IPV4:
                # Lots of stuff to check, so call private helper function to
                # handle.
    ...
    def to_binary(self):
        """
        Convert field to binary string with length of self.size.

        Return:
            out : str
            Binary string of field.
        """
        if type(self.value) is int:
            # Integer value. Just convert to binary with bin() and remove the
            # 0b part at the start.
            t = bin(self.value).lstrip("0b")
            if len(t) == self.size:
                return t
            elif self.size > len(t):
                return t + "0" * (self.size-len(t))
        elif self.value_type == "bytes" and type(self.value) is bytes:
            ...
            for b in self.value:
                ...
                # add an _ to separate each group of 8 bits.
                # We will have an extra _ at the end, so we will remove it
                # later.
                m += '0'*(8-len(a)) + a + "_"
            t = m[:-1]  # Remove the last _ of the bit string.

            if len(self.__remove_all(t, "_")) == self.size * 8:
                return t
            elif self.size * 8 > len(self.__remove_all(t, "_")):
                return (t + "00000000_" * (self.size-len(t)))[:-1]
        elif self.value_type in SPECIAL_FIELD_TYPES and type(self.value) is bytes:
            # Do the same thing as bytes. Loop through the bytes, and
            # convert them to binary and remove the 0b at the start.
            m = ''
            for b in self.value:
                a = bin(b).lstrip("0b")
                # If the binary number of the current byte is not 8 bits,
                # add all a padding of 0s at the start to make it 8 bits,
                # and then add an _ to separate each group of 8 bits.
                # We will have an extra _ at the end, so we will remove it
                # later.
                m += '0' * (8 - len(a)) + a + "_"
    ...
    def to_hex(self):
        """
        Convert field to hex string of length self.size / 4.
    ...
    def __str__(self):
        ...
        This string representation is of the form:
        ...
        if type(self.value) is bytes:
            return 'Field(name: {}, size: {}, value: '.format(self.name, self.size) + self.value.decode() + ")"
        else:
            return 'Field(name: {}, size: {}, value: {})'.format(self.name, self.size, self.value)

    def __can_fit(self, size, value):
        """
        Make sure the specified value can fit in the specified number of bits.
        ...
        value : (int)
                The value to see if it can fit in the number of bits.
        """
        return value.bit_length() <= size

    def __handle_ipv4(self, value):
        """
        Handle the creation of an IPv4 address field.

        Make sure it is a valid address, and if so, store it in self.value.
        Also make self.size = 32 since IPv4 addresses are 32-bits (4 bytes).

        Parameters:
        value : (int, str, or tuple)
                The value to determine if it is a valid IPv4 address, and
                if so, set self.value to it.
        """
        if value == 0:
            # If the value is 0, this field will be a placeholder waiting to be
            # filled in the future. It must be filled before packetize() is
            # called.
            self.size = -1
            self.value = "undefined"
            ...
            if len(value.split(".")) != 4:
                raise InvalidIPv4Address(value,
                      "IPv4 Addresses must have 4 numbers.")
            ...
            for v in value.split("."):
                ...
                if v_int < 0 or v_int > 255:
                    # IPv4 address is not valid with a negative number or one greater than 255.
            ...
            if len(value) != 4:
                raise InvalidIPv4Address(value,
                      "IPv4 Addresses must have 4 numbers.")
            self.size = 32  # IPv4 have 32 bits.
            # Loop through each value, make sure it is a valid number, and if
            # so, add it to the value.
            self.value = b''
            for v in value:
                if v < 0 or v > 255:
                    ...
                    # greater than 255.
                    raise InvalidIPv4Address(value,
                          "IPv4 numbers must be between 0-255.")
                else:
                    # Valid number. Add the byte-encoded number to the value.
    ...
    def __handle_prefix_len(self, value):
        """
        Handle any values that need length to be prefixed to it.

        Can be a single value in which the length of the value is added to
        start, or it can be a list of values in which the length of each value
        is added before it.

        Parameters:
            value : (str or tuple)
                    The value, or values to add the length(s) to.
        """
        if type(value) is str:
            ...
            # Currently only support a string or tuple value. If it is
            # something else, throw an error.
            raise InvalidValue(type(value))
        self.value_type = PREFIX_LENGTH

    def __remove_all(self, string, character_to_remove):
        """Remove all occurrences of specified character of string.

        Parameters:
            string : (str)
                    The string to remove the characters of.
            character_to_remove : (char)
                                   The specified character to remove."""
        msg = ""
        for c in string:
            if character_to_remove != c:
                msg += c
        return msg
